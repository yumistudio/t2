	...
	if ( 'top' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ($depth === 0 && 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . twentyseventeen_get_svg( array( 'icon' => 'chevron' ) );
			}
		}

		if ($depth === 1 && intval($item->object_id) && $icon = get_field('p_icon', $item->object_id)) {
			$title = twentyseventeen_get_svg(array('icon'=> $icon)) . $title;
					  '</svg>'. $title;
		}

		if (($description = get_field('navigation_intro', $item->object_id)) && $depth < 2 && intval($item->object_id) != 226) {
			$title = $title . '<div class="navi-desc">'.$description.'</div>';
		}
	}
	return $title;
}
